...
const MergeScheduleUnits = (props) => {
    ...
    const handleSave = () => {
        let finalSize = selectedScheduleUnits.map(unit => unit.Size).reduce((prev, curr) => prev + curr, 0);
        let isUnitSizeThere = unitSizesData.find(x => parseInt(x.unitSize) === finalSize);
        if (!isUnitSizeThere) {
            let availSizes = unitSizesData.map(x => x.unitSize).join();
            notifyWarning(AppLanguage.APP_MESSAGE.Unavailable_Unit.replace('__availSizes__', availSizes));
            return false;
        }
        ...
        MergeScheduleUnitsAPI(obj).then(data => {
            notifySuccess(AppLanguage.APP_MESSAGE.Merge_Schedule_Unit);
            props.handleSave();
        }).catch(err => {
            console.log(err);
        })
                    ...
                    <Grid container spacing={0.5} ml={2} mr={2}>
                        ...
                        {props.SameTypeUnits.map(unit => {
                            return (<>
                                ...
                                <Grid item xs={6}>
                                    <Typography variant="body2">{unit.planName ?? '-'}</Typography>
                                </Grid>
                                <Grid item xs={2}>
                        ...
                        <Grid item xs={2}>
                            <Typography variant="body2">{selectedScheduleUnits.map(unit => unit.Size).reduce((prev, curr) => prev + curr, 0)}</Typography>
                        </Grid>
                    </Grid>
                </Grid>
