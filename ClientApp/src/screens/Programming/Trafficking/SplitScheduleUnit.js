...
const SplitScheduleUnit = (props) => {
    ...
    const handleSave = () => {
        ...
        if(parseInt(orgSize) !== finalSize)
        {
            notifyWarning('Sum of count is not equal to Orginal size ' + orgSize);
            return false;
        }
        ...
        SplitScheduleUnitAPI(objList).then(data => {
            notifySuccess('Selected Unit splitted successfully');
            props.handleSave();
        }).catch(err => {
            console.log(err);
        })
            ...
            <Grid container my={2}>
                ...
                <Grid item xs={12}>
                    <Grid container spacing={1.5} ml={2} mr={2}>
                        ...
                        {unitSizesData.filter(x => parseInt(x.unitSize) < props.Schedule?.unitSize).map(size =>{
                            return(<>
                                <Grid item xs={6}>
